...
if(now>=start+options.timeout){options.error();return false;}window.setTimeout(fn,10);};window.setTimeout(fn,10);},toggleQuality:function(img,force){if((IE!==7&&IE!==8)||!img){return;}
...
callback=callback||function(){};if(link){callback.call(link,link);return link;}length=doc.styleSheets.length;if(DEBUG){href+='?'+Utils.timestamp();}
...
if(!idle.bound){idle.addEvent();}elem=$(elem);var from={},style;for(style in to){if(to.hasOwnProperty(style)){from[style]=elem.css(style);}}
...
if(options&&typeof options.imageTimeout==='number'){TIMEOUT=options.imageTimeout;}if(options&&typeof options.dummy==='string'){DUMMY=options.dummy;}
...
self.trigger(Galleria.PLAY);self._playing=true;}if(self._firstrun){if(typeof self._options.show==='number'){self.show(self._options.show);}return;}
...
self.next();});}if(Galleria.History){Galleria.History.change(function(value){if(isNaN(value)){window.history.go(-1);}else{self.show(value,undef,true);}});}
...
return this;}$(source).find(selector).each(function(i,img){img=$(img);var data={},parent=img.parent(),href=parent.attr('href'),rel=parent.attr('rel');if(href){data.image=data.big=href;}
if(rel){data.big=rel;}self._data.push($.extend({title:img.attr('title')||'',thumb:img.attr('src'),image:img.attr('src'),big:img.attr('src'),description:img.attr('alt')||'',link:img.attr('longdesc'),original:img.get(0)},data,config(img)));});if(this.getDataLength()){this.trigger(Galleria.DATA);}else{Galleria.raise('Load failed: no data found.');}
...
return;}self.openLightbox();});}Array.prototype.shift.call(self._queue);if(self._queue.length){self._show();}
...
return c/2*((t-=2)*t*t+2)+b;},galleriaIn:function(_,t,b,c,d){return c*(t/=d)*t+b;},galleriaOut:function(_,t,b,c,d){return-c*(t/=d)*(t-2)+b;}});$.fn.galleria=function(options){return this.each(function(){$(this).data('galleria',new Galleria().init(this,options));});};}(jQuery));